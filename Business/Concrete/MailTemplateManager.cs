...
namespace Business.Concrete
{
    public class MailTemplateManager : IMailTemplateService
    {
        ...
        public IDataResult<List<MailTemplate>> GetAll(int id)
        {
            return new SuccessDataResult<List<MailTemplate>>(_mailTemplateDal.GetList(m => m.Id == id));
        }
       
        public IDataResult<MailTemplate> GetByTemplateName(string name)
        {
            return new SuccessDataResult<MailTemplate>(_mailTemplateDal.Get(m => m.Type == name ));
        }
